  ...
  $database_to_backup.each |Integer $index, Boolean $value | {
    ...
    out::message("# Backing up database ${database_names[$index]}")
      ...
      if $database_names[$index] == 'pe-puppetdb' and $cluster['primary_postgresql_host'] {
        ...
        run_command("mv ${database_backup_directory}/puppetdb_*.bin ${output_directory}/", $primary_host )
      } else {
        ...
        run_command("mv ${database_backup_directory}/${database_names[$index]}*.bin ${output_directory}/", $primary_host )
      }
    }
  }
