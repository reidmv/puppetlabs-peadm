  ...
  $database_to_restore.each |Integer $index, Boolean $value | {
    ...
    out::message("# Restoring database ${database_names[$index]}")
      ...
      if $database_names[$index] == 'pe-puppetdb' and $primary_postgresql_host {
        ...
        # Restore database
        run_command("cp -p ${backup_directory}/${database_names[$index]}_*.bin ${database_backup_directory}/ ", $primary_host )
        ...
        run_command("sudo -hu pe-postgres /opt/puppetlabs/server/bin/pg_restore -d ${database_names[$index]} -c ${backup_directory}/${database_names[$index]}_*.bin",$primary_host) # lint:ignore:140chars
